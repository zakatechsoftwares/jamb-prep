...
4. Automated gates run before any human sees the item: schema validation, duplicate detection by embedding similarity against the live bank, key-distribution rebalancing, rejection of 'all of the above' and 'none of the above', rejection of length-outlier options, and distractor-rationale completeness — every one of the three wrong options must carry a distractor_rationale naming its specific error. An item missing any of the three is held at gate_failed until the rationale is supplied; it cannot advance by any route, automated or human.
...
7. Approved items — by either route — go live as 'approved_uncalibrated' and accumulate response data, from which empirical difficulty and discrimination are computed. No item reaches 'approved_uncalibrated' without distractor_rationale recorded on all three wrong options; that gate in step 4 is a precondition of this step, not a suggestion.
...
| Queue prioritisation | (1) Items where the blind second solve disagreed with the proposed key, (2) high risk_tier items, (3) items in needs_second_review, (4) items flagged by other automated gates, (5) low-risk sampling, (6) human-authored contributions, which queue alongside the low-risk sample in band 5. A disagreed verdict is evidence this specific item is probably wrong; high risk_tier only means the category is risky. A concrete suspected error outranks a merely risky category. A commissioned item from a known author is a known quantity, so it waits behind suspected defects rather than ahead of them |
