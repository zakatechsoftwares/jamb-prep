 ...
 * (plan 7.11) — a no-op otherwise. Called from `decideOnItem`'s own
 * transaction, right after the decision itself is recorded, for both a
 * `'live'` and a `'late_arrival'` decision alike (a late-arriving second
 * opinion on a gold item is still a genuine accuracy data point).
 *
...
export async function scoreGoldDecision(
  ...
  const accuracy = firstRow(
    await client.query<{ accuracy_score: string }>(
